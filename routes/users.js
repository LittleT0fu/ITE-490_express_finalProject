...
router.post('/register', [
    body('name').trim().not().isEmpty().withMessage("กรุณาป้อนชื่อด้วย"),
    body('email').trim().not().isEmpty().withMessage("กรุณาป้อนอีเมลล์").isEmail().withMessage("รูปแบบอีเมลล์ไม่ถูกต้อง"),
    body('password').trim().not().isEmpty().withMessage("กรณาใส่พาสเวิร์ด").isLength({ min: 5 }).withMessage("รหัสผ่านต้องมีค่ามากกว่า 5 ตัวอักษรขึ้นไป")
